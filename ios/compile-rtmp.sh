#! /usr/bin/env bash
#
# Copyright (C) 2013-2014 HatsuneMikuV <anglemiku_v@163.com>
#
...
do_lipo () {
    ...
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/rtmp-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    cp -R $UNI_BUILD_ROOT/build/rtmp-armv7/output/include $UNI_BUILD_ROOT/build/universal/
}

#----------
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh tools/do-compile-rtmp.sh $FF_TARGET
    ...
    echo "Usage:"
    echo "  compile-rtmp.sh armv7|arm64|i386|x86_64"
    echo "  compile-rtmp.sh armv7s (obselete)"
