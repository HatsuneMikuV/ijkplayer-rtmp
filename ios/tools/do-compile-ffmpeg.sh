...
if [ "$FF_ARCH" = "i386" ]; then
    ...
    FF_BUILD_NAME_RTMP=rtmp-x86_64
    FF_XCRUN_PLATFORM="iPhoneSimulator"
...
#    FFMPEG_CFG_CPU="--cpu=cortex-a8"
    ...
    FF_BUILD_NAME_RTMP=rtmp-armv7s
...
echo "[*] check rtmp"
echo "----------------------"
FFMPEG_DEP_RTMP_INC=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMP/output/include
FFMPEG_DEP_RTMP_LIB=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMP/output/lib
#--------------------
# with rtmp
if [ -f "${FFMPEG_DEP_RTMP_LIB}/librtmp.a" ]; then
FFMPEG_CFG_FLAGS="$FFMPEG_CFG_FLAGS --enable-librtmp"

FFMPEG_CFLAGS="$FFMPEG_CFLAGS -I${FFMPEG_DEP_RTMP_INC}"
FFMPEG_DEP_LIBS="$FFMPEG_DEP_LIBS -L${FFMPEG_DEP_RTMP_LIB} -lrtmp"
fi
